...
int mute_mask = 0;
int mute_mask_special_operators = 0;
int channelsOutput[64] = {0};
int newVol = 0;
//*********************************************************
//  OPM SPECIFICS
//*********************************************************
...
template<class RegisterType>
s32 ymfm_operator<RegisterType>::compute_volume(u32 phase, u32 am_offset, int chn) const
{
	newVol += attenuation_to_volume(envelope_attenuation(am_offset) << 2);
...
template<class RegisterType>
s32 ymfm_operator<RegisterType>::compute_noise_volume(u32 am_offset, int chn) const
{
...
void ymfm_channel<RegisterType>::reset()
{
	...
	m_feedback_in = 0;
	for(int i=0; i<64; i++)channelsOutput[i]=0;
}
...
template<class RegisterType>
void ymfm_channel<RegisterType>::output_4op(int chn, s32 outputs[RegisterType::OUTPUTS], u32 rshift, s32 clipmax, u32 mute_mask_operators) const
{
	...
	newVol = 0;
	// compute the 14-bit volume/value of operator 1 and update the feedback
	s32 op1value = m_feedback_in = m_op[0]->compute_volume(m_op[0]->phase() + opmod, am_offset, chn);
	...
	opout[2] = m_op[1]->compute_volume(m_op[1]->phase() + opmod, am_offset, chn);
	opout[5] = opout[1] + opout[2];

	// compute the 14-bit volume/value of operator 3
	opmod = opout[BIT(algorithm_ops, 1, 3)] >> 1;
	opout[3] = m_op[2]->compute_volume(m_op[2]->phase() + opmod, am_offset, chn);
	...
	if (m_regs.noise_enable() && m_choffs == 7){
		if(mute_mask_operators&0x08)
			result = m_op[3]->compute_noise_volume(am_offset, chn);
	}
	else
	{
		opmod = opout[BIT(algorithm_ops, 4, 3)] >> 1;
		if(mute_mask_operators&0x08)
			result = m_op[3]->compute_volume(m_op[3]->phase() + opmod, am_offset, chn);
	}
	result >>= rshift;

	// optionally add OP1, OP2, OP3
	s32 clipmin = -clipmax - 1;
	if (BIT(algorithm_ops, 7) != 0 && mute_mask_operators&0x01)
		result = std::clamp(result + (opout[1] >> rshift), clipmin, clipmax);
	if (BIT(algorithm_ops, 8) != 0 && mute_mask_operators&0x02)
		result = std::clamp(result + (opout[2] >> rshift), clipmin, clipmax);
	if (BIT(algorithm_ops, 9) != 0 && mute_mask_operators&0x04)
		result = std::clamp(result + (opout[3] >> rshift), clipmin, clipmax);

	channelsOutput[chn] = newVol;
	// add to the output
	add_to_output(m_choffs, outputs, result);
}
...
template<class RegisterType>
ymfm_engine_base<RegisterType>::ymfm_engine_base() :
	m_env_counter(0),
	m_status(0),
...
template<class RegisterType>
void ymfm_engine_base<RegisterType>::output(s32 outputs[RegisterType::OUTPUTS], u32 rshift, s32 clipmax, u32 chanmask, u32 mute_mask, u32 channel_special_mute_operators, u32 mute_mask_per_operator) const
{
		...
		for (int chnum = 0; chnum < CHANNELS; chnum++)
			if (BIT(chanmask, chnum))
			{
				if (chnum == 6)
					...
					m_channel[chnum]->output_4op(chnum, outputs, rshift, clipmax, channel_special_mute_operators==chnum?mute_mask_per_operator:0xFF);
				else
	...
	else
	{
		// sum over all the desired channels
		for (int chnum = 0; chnum < CHANNELS; chnum++)
			if (BIT(chanmask, chnum) && (mute_mask&(1<<chnum)))
			{
...
void ymfm_engine_base<RegisterType>::write(u16 regnum, u8 data)
{
	...
	if (m_regs.write(regnum, data, keyon_channel, keyon_opmask))
	{
		...
		if (keyon_channel < CHANNELS)
		{
			...
			m_channel[keyon_channel]->prepare();
		}

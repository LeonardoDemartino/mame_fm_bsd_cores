...
ymadpcm_a_channel::ymadpcm_a_channel(ymadpcm_a_engine &owner, u32 choffs, u32 addrshift) :
	m_choffs(choffs),
...
bool ymadpcm_a_channel::clock(unsigned char *memory, unsigned int size)
{
	...
	if (m_curnibble == 0)
	{
		if(m_curaddress<size){
			m_curbyte = memory[m_curaddress++];
		}
		data = m_curbyte >> 4;
		m_curnibble = 1;
	}
...
ymadpcm_a_engine::ymadpcm_a_engine(u32 addrshift)
{
	// create the channels
	for (int chnum = 0; chnum < CHANNELS; chnum++)
		m_channel[chnum] = std::make_unique<ymadpcm_a_channel>(*this, chnum, addrshift);
}
...
unsigned char *adpcma_memory = NULL;
unsigned int adpcma_size = 0;
...
u32 ymadpcm_a_engine::clock(u32 chanmask)
{
	...
	for (int chnum = 0; chnum < std::size(m_channel); chnum++)
		if (BIT(chanmask, chnum)){
			if (m_channel[chnum]->clock(adpcma_memory, adpcma_size))
				result |= 1 << chnum;
			channelsOutput[6+3+chnum] = abs((int)m_channel[chnum]->m_curbyte)/10.0f;
		}
		else channelsOutput[6+3+chnum] = 0;

	// return the bitmask of completed samples
	return result;
}

void ymadpcm_a_engine::set_memory(unsigned char *memory, unsigned int size)
{
	adpcma_memory = memory;
	adpcma_size = size;
}
...
void ymadpcm_a_engine::output(s32 outputs[2], u32 chanmask, int mute_mask)
{
	// compute the output of each channel
	for (int chnum = 0; chnum < std::size(m_channel); chnum++)
		if (BIT(chanmask, chnum) && (mute_mask&(1<<(9+chnum))))
...
ymadpcm_b_channel::ymadpcm_b_channel(ymadpcm_b_engine &owner, u32 addrshift) :
	m_address_shift(addrshift),
...
void ymadpcm_b_channel::clock()
{
	...
	if (m_regs.external())
	{
		...
		if (m_curnibble == 0)
		{
			m_curbyte = 0 /*TODO: Pointer from defle memory*/;
...
u8 ymadpcm_b_channel::read(u32 regnum)
{
	...
	if (regnum == 0x08 && !m_regs.execute() && !m_regs.record() && m_regs.external())
	{
		...
		{
			result = 0 /*TODO: Pointer from defle memory*/;
			m_status = STATUS_BRDY;
		}
	}
	return result;
...
ymadpcm_b_engine::ymadpcm_b_engine(u32 addrshift)
{
	// create the channel (only one supported for now, but leaving possibilities open)
	m_channel[0] = std::make_unique<ymadpcm_b_channel>(*this, addrshift);
}
...
void ymadpcm_b_engine::output(s32 outputs[2], u32 rshift, u32 chanmask, int mute_mask)
{
	// compute the output of each channel
	for (int chnum = 0; chnum < std::size(m_channel); chnum++)
		if (BIT(chanmask, chnum) && (mute_mask&(1<<(9+7+chnum))))
